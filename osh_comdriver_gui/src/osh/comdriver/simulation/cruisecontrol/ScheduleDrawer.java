...
public class ScheduleDrawer extends JPanel {
    ...
    public ScheduleDrawer(
        ...
        this.panel = createDemoPanel(schedules, ps, pls, currentTime);
...
//			int cntr = 1;
            ...
            for (Schedule i : schedules) {
                ...
                XYSeries[] powerProfile = renderSeries(i.getProfile(), i.getScheduleName(), currentTime);

                for (XYSeries xySeries : powerProfile) {
                    dataset3.addSeries(xySeries);
                }

                powerSum = (SparseLoadProfile) powerSum.merge(i.getProfile(), 0);
            }

            XYSeries[] powerProfileSum = renderSeries(powerSum, "sum", currentTime);

            for (XYSeries xySeries : powerProfileSum) {
                dataset3.addSeries(xySeries);
            }
        }
    ...
    private static JFreeChart createStuffForPanel(
        ...
        return createChart(dataSets[0], dataSets[1], dataSets[2], time);
    }
        ...
        if (!saveGraph) {
            ...
            try {
                out = new BufferedOutputStream(
                        new FileOutputStream("logs/graphic_" + this.counter + ".png"));
                ChartUtils.writeChartAsPNG(out, chart, 1024, 768);
                out.close();
            } catch (IOException e) {
                e.printStackTrace();
            }
