...
public class AdsorptionChillerModel {
    ...
    private static double outdoorToCondenserTemperature(double z) {
        return Math.max(0.8781 * z + 4.365, z + 1);
    }
    ...
    public static double cop(double x, double z) {
        ...
        double condenserTemperature = outdoorToCondenserTemperature(z);
        ...
        if (condenserTemperature < 22) {

            a = copHelp(x, 22);

            result = a;
        }
        if (condenserTemperature >= 22 && condenserTemperature < 27) {

            a = copHelp(x, 22);
            b = copHelp(x, 27);

            w1 = (condenserTemperature - 27) / (-5);
            w2 = 1 - w1;

            result = w1 * a + w2 * b;
        } else if (condenserTemperature >= 27 && condenserTemperature < 32) {

            a = copHelp(x, 27);
            b = copHelp(x, 32);

            w1 = (condenserTemperature - 32) / (-5);
            w2 = 1 - w1;

            result = w1 * a + w2 * b;
        } else if (condenserTemperature >= 32 && condenserTemperature <= 37) {

            a = copHelp(x, 32);
            b = copHelp(x, 37);

            w1 = (condenserTemperature - 37) / (-5);
            w2 = 1 - w1;

            result = w1 * a + w2 * b;
        } else if (condenserTemperature > 37) {
