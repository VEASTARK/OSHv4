	...
	public void onDeviceStateUpdate() throws OCUnitException {
		...
		if (hx instanceof HotWaterDemandObserverExchange) {
			...
			if (lastTimeFromMidnight <= this.timeFromMidnight && now % 3600 == 0) {
				double predVal = predictedWaterDemand.getLoadAt(Commodity.DOMESTICHOTWATERPOWER, timeFromMidnight);
				...
				if ((predVal != 0 && ((double) hotWaterPower / predVal  > 1.25 
					...
					int oldVal = predictedWaterDemand.getLoadAt(Commodity.DOMESTICHOTWATERPOWER, timeFromMidnight);
					Long nextLoadChange = predictedWaterDemand.getNextLoadChange(Commodity.DOMESTICHOTWATERPOWER, timeFromMidnight);
					...
					if ((nextLoadChange != null && nextLoadChange > timeFromMidnight + 3600) 
						...
						predictedWaterDemand.setLoad(Commodity.DOMESTICHOTWATERPOWER, timeFromMidnight + 3600, oldVal);
					}
					predictedWaterDemand.setLoad(Commodity.DOMESTICHOTWATERPOWER, timeFromMidnight, hotWaterPower);
					sendIPP();
				}				
			}				
