...
public class PvProfileHollSingleton {
    ...
    public PvProfileHollSingleton(
        ...
        this.init(pathToProfile, nominalPower, profileNominalPower);
    }
    ...
    public SparseLoadProfile getPowerForDay(int dayOfYear) {
        int correctedDayOfYear = dayOfYear;
        if (correctedDayOfYear > 365) {
            correctedDayOfYear = 365;
        }

        SparseLoadProfile slp = new SparseLoadProfile();

        for (int minute = 0; minute < 1440; minute++) {
            int activePower = pvProfileArray[correctedDayOfYear][minute];
            // to be safe...
            if (activePower > 0) {
