...
public interface ILoadProfile<T extends Enum<T>> {
     ...
     * @return duration
     */
    long getEndingTimeOfProfile();

    int getLoadAt(T commodity, long t);

    Long getNextLoadChange(T commodity, long t);

    ILoadProfile<T> merge(ILoadProfile<T> other, long offset);
     ...
     */
    void cutOffNegativeTimeValues();

    ILoadProfile<T> clone();
