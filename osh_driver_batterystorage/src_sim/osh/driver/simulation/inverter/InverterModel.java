...
public abstract class InverterModel implements Serializable {
    ...
    public int calcBatteryChargePower(int availablePower) { //current power is negative
        return -1 * (int) Math.round((availablePower - Math.signum(this.inverterBasePower)) * (this.getInverterEfficiency())); // charge power is positive
    }
...
//	}
    ...
    public int calcActivePowerOutputChargingState(double batteryChargePower) { // charge power is positive
        return (int) Math.round(((batteryChargePower)) / (this.getInverterEfficiency()) + Math.signum(batteryChargePower) * this.inverterBasePower);
    }
