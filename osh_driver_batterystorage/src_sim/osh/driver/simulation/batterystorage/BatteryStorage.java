...
public abstract class BatteryStorage {
    ...
    //Room Temperature
    public final double roomTemperature;
    protected final double efficiencyFactor = 1.0;
    ...
    public void reduceByStandingLoss(long seconds) {
        int standingEnergyLoss = this.calcStandingLoss(seconds);
        this.changeSOC(standingEnergyLoss);
    ...
    private int calcStandingLoss(long seconds) {
        return (int) (this.standingLoss * seconds);
    }
     ...
     */
    public double getSlowChargePowerInterval() {
     ...
     * negative value: remove energy from battery storage
     *
     * @param deltaEnergy [Ws] (positive value: add energy to battery storage)
     */
    public void changeSOC(int deltaEnergy) {
        this.stateOfCharge += deltaEnergy; // negative, because chargePower is negative and dischargePower is positive
    }
