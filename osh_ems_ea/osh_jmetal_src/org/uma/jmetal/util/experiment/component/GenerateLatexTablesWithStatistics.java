...
public class GenerateLatexTablesWithStatistics implements ExperimentComponent {
    ...
    private List<List<List<List<Double>>>> readDataFromFiles() throws IOException {
        List<List<List<List<Double>>>> data = new ArrayList<>(this.experiment.getIndicatorList().size());
        ...
        for (int indicator = 0; indicator < this.experiment.getIndicatorList().size(); indicator++) {
            ...
            for (int problem = 0; problem < this.experiment.getProblemList().size(); problem++) {
                ...
                for (int algorithm = 0; algorithm < this.experiment.getAlgorithmList().size(); algorithm++) {
                    ...
                    String directory = this.experiment.getExperimentBaseDirectory();
                    directory += "/data/";
                    directory += "/" + this.experiment.getAlgorithmList().get(algorithm).getAlgorithmTag();
                    directory += "/" + this.experiment.getProblemList().get(problem).getTag();
                    directory += "/" + this.experiment.getIndicatorList().get(indicator).getName();
    ...
    private void generateLatexScript(List<List<List<List<Double>>>> data) throws IOException {
        String latexDirectoryName = this.experiment.getExperimentBaseDirectory() + "/" + DEFAULT_LATEX_DIRECTORY;
        File latexOutput;
    ...
    private void printData(String latexFile, int indicatorIndex, double[][][] centralTendency, double[][][] dispersion, String caption) throws IOException {
        ...
        try (FileWriter os = new FileWriter(latexFile, true)) {
            ...
            os.write(StringUtils.repeat("l", this.experiment.getAlgorithmList().size()));
            os.write("}\n");
            os.write("\\hline");
            ...
            for (int i = -1; i < this.experiment.getAlgorithmList().size(); i++) {
                if (i == -1) {
                    os.write(" & ");
                } else if (i == (this.experiment.getAlgorithmList().size() - 1)) {
            ...
            for (int i = 0; i < this.experiment.getProblemList().size(); i++) {
                ...
                if (this.experiment.getIndicatorList().get(indicatorIndex).isTheLowerTheIndicatorValueTheBetter()) {
                    ...
                    for (int j = 0; j < (this.experiment.getAlgorithmList().size()); j++) {
                ...
                for (int j = 0; j < (this.experiment.getAlgorithmList().size() - 1); j++) {
                    if (j == bestIndex) {
                ...
                String m = String.format(Locale.ENGLISH, "%10.2e",
                        centralTendency[indicatorIndex][i][this.experiment.getAlgorithmList().size() - 1]);
                String s = String.format(Locale.ENGLISH, "%8.1e",
                        dispersion[indicatorIndex][i][this.experiment.getAlgorithmList().size() - 1]);
                os.write("$" + m + "_{" + s + "}$ \\\\" + "\n");
            }
