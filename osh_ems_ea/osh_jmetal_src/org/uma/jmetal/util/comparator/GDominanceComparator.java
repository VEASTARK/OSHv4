...
public class GDominanceComparator<S extends Solution<?>> implements Comparator<S>, Serializable {
    ...
    public int compare(S solution1, S solution2) {
        ...
        return this.flagComparison(solution1, solution2);
    }

    private int flagComparison(S solution1, S solution2) {
        int result;
        if (this.flag(solution1) > this.flag(solution2)) {
            result = -1;
        } else if (this.flag(solution1) < this.flag(solution2)) {
            result = 1;
        } else {
            result = this.dominanceComparator.compare(solution1, solution2);
        }
    ...
    private int flag(S solution) {
        ...
        for (int i = 0; i < solution.getNumberOfObjectives(); i++) {
            if (solution.getObjective(i) > this.referencePoint.get(i)) {
                result = 0;
            }
        }
        if (result == 0) {
