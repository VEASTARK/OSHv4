...
public class MeasureFactory {
    ...
    public <Value> PushMeasure<Value> createPushFromPull(
        ...
        SimplePushMeasure<Value> push = new SimplePushMeasure<>(pull.getName(),
                pull.getDescription());
        final WeakReference<PullMeasure<Value>> weakPull = new WeakReference<>(
                pull);
        final WeakReference<SimplePushMeasure<Value>> weakPush = new WeakReference<>(
                push);
            ...
            public void run() {
                ...
                while (isThreadNeeded) {
                    if (alreadyConsumed > period) {
                        ...
                        alreadyConsumed %= period;
                        MeasureFactory.this.log.warning("Too much time consumed in the last measuring ("
                                + realConsumption
                                + ">"
                                + period
                                + "), ignore the "
    ...
    public Map<String, PullMeasure<?>> createPullsFromGetters(
            final Object object) {
        Map<String, PullMeasure<?>> measures = new HashMap<>();
        ...
        for (final Method method : clazz.getMethods()) {
            if (method.getParameterTypes().length == 0
                ...
                measures.put(key, new SimplePullMeasure<>(key) {
