...
public class StrengthRawFitness<S extends Solution<?>>
    ...
    public void computeDensityEstimator(List<S> solutionSet) {
        ...
        for (int i = 0; i < solutionSet.size(); i++) {
            for (S s : solutionSet) {
                if (DOMINANCE_COMPARATOR.compare(solutionSet.get(i), s) < 0) {
                    strength[i] += 1.0;
                }
            }
        }
        ...
        for (int i = 0; i < distance.length; i++) {
            ...
            kDistance = 1.0 / (distance[i][this.k] + 2.0);
            solutionSet.get(i).setAttribute(this.getAttributeIdentifier(), rawFitness[i] + kDistance);
        }
    }
