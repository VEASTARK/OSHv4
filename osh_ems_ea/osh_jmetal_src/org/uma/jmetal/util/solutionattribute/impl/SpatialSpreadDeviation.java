    ...
    public void computeDensityEstimator(List<S> solutionList) {
        ...
        if (size <= solutionList.get(0).getNumberOfObjectives()) {
            for (S s : solutionList) {
                s.setAttribute(this.getAttributeID(), Double.POSITIVE_INFINITY);
            }
            return;
        }
        ...
        for (int i = 0; i < size; i++) {
            front.get(i).setAttribute(this.getAttributeID(), 0.0);
        }
        ...
        for (int i = 0; i < numberOfObjectives; i++) {
            ...
            front.get(0).setAttribute(this.getAttributeID(), Double.POSITIVE_INFINITY);
            front.get(size - 1).setAttribute(this.getAttributeID(), Double.POSITIVE_INFINITY);
        }
        double[][] distance =
        ...
        for (int i = 0; i < front.size(); i++) {
            ...
            temp = Math.sqrt(temp);
            temp *= -1;
            temp += (double) front.get(i).getAttribute(this.getAttributeID());
            // if((double) front.get(i).getAttribute(getAttributeID())!=Double.POSITIVE_INFINITY)
            front.get(i).setAttribute(this.getAttributeID(), temp);
        }
        ...
        for (int i = 0; i < distance.length; i++) {
            ...
            double temp = (double) front.get(i).getAttribute(this.getAttributeID());
