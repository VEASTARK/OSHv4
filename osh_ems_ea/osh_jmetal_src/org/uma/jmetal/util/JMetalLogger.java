...
public class JMetalLogger implements Serializable {
    ...
    public static void configureLoggers(File propertyFile) throws IOException {
        ...
        if (propertyFile != null) {
            printer.println(FileUtils.readFileToString(propertyFile, StandardCharsets.UTF_8));
        } else if (defaultFile.exists()) {
            printer.println(FileUtils.readFileToString(defaultFile, StandardCharsets.UTF_8));
        } else {
        ...
        manager.readConfiguration(IOUtils.toInputStream(new String(stream
                .toByteArray(), StandardCharsets.UTF_8), StandardCharsets.UTF_8));
