...
public class EnergyArchive<S extends Solution<?>> extends AbstractBoundedArchive<S> {
    ...
    public void computeDensityEstimator() {
        ...
        for (int i = 0; i < replacementVector.length; i++) {
            ...
            if (replacementVector[i] < energyVector[i]) {
                ...
                this.archive.get(i).setAttribute(this.fitness.getAttributeIdentifier(), -Double.MAX_VALUE);
            }
            if (eligible) {
    ...
    public void sortByDensityEstimator() {
        this.getSolutionList().sort(this.fitnessComparator);
    }
    ...
    public void prune() {
        if (this.getSolutionList().size() > this.getMaxSize()) {
            ...
            S worst = new SolutionListUtils().findWorstSolution(this.getSolutionList(), this.fitnessComparator);
    ...
    private void scaleToPositive() {
        ...
        for (S solution : this.getSolutionList()) {
            if (this.scalarization.getAttribute(solution) < minScalarization) {
                minScalarization = this.scalarization.getAttribute(solution);
            }
        }
        if (minScalarization < 0) {
            ...
            double eps = 10.0e-6;
            for (S solution : this.getSolutionList()) {
                this.scalarization.setAttribute(solution, eps + this.scalarization.getAttribute(solution) + minScalarization);
            }
        }
    }
    ...
    private double[] energyVector(double[][] distanceMatrix) {
        ...
        for (int i = 0; i < energyVector.length - 1; i++) {
            for (int j = i + 1; j < energyVector.length; j++) {
                energyVector[i] += this.scalarization.getAttribute(this.archive.get(j)) / distanceMatrix[i][j];
                energyVector[j] += this.scalarization.getAttribute(this.archive.get(i)) / distanceMatrix[i][j];
            }
            energyVector[i] *= this.scalarization.getAttribute(this.archive.get(i));
        }
        return energyVector;
    }
    ...
    private double[] replacementVector(double[][] distanceMatrix) {
        ...
        for (int i = 0; i < replacementVector.length; i++) {
            individualEnergy[i] = this.scalarization.getAttribute(this.archive.get(i)) / distanceMatrix[i][this.maxSize];
            totalEnergy += individualEnergy[i];
        }
        for (int i = 0; i < individualEnergy.length; i++) {
            replacementVector[i] = totalEnergy - individualEnergy[i];
            replacementVector[i] *= this.scalarization.getAttribute(this.archive.get(this.maxSize));
        }
