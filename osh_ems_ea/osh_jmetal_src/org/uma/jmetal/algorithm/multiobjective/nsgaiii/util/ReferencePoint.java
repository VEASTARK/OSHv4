...
public class ReferencePoint<S extends Solution<?>> {
    ...
    public ReferencePoint(ReferencePoint<S> point) {
        this.position = new ArrayList<>(point.position.size());
        this.position.addAll(point.position);
        this.memberSize = 0;
        this.potentialMembers = new ArrayList<>();
    }
    ...
    public boolean HasPotentialMember() {
        return !this.potentialMembers.isEmpty();
    }
    ...
    public void AddPotentialMember(S member_ind, double distance) {
        this.potentialMembers.add(new ImmutablePair<>(member_ind, distance));
    }
