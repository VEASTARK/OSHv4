...
public class StochasticUniversalSampling extends Selection {
    ...
    private final int numberToSelect_;
    ...
    public StochasticUniversalSampling(HashMap<String, Object> parameters, PseudoRandom pseudoRandom) throws JMException {
        ...
        if ((parameters != null) && (parameters.get("numberToSelect") != null)) {
            this.numberToSelect_ = (int) parameters.get("numberToSelect");
            if (this.numberToSelect_ < 1) {
                throw new JMException("Number of to solutions to select cannot be zero or negative");
            }
        } else
            this.numberToSelect_ = 2;
    ...
    public Object execute(Object object) {
        ...
        Solution[] result = new Solution[this.numberToSelect_];
        solutionSet.sort(this.comparator_);
        ...
        if (sumOfFitness == 0) {
            for (int i = 0; i < this.numberToSelect_; i++) {
                result[i] = solutionSet.get(this.pseudoRandom.randInt(0, solutionSet.size()));
            }
            return result;
        }
        ...
        double[] selectedValues = new double[this.numberToSelect_];
        double valueDistance = 1.0d / this.numberToSelect_;
        double firstValue = this.pseudoRandom.randDouble(0.0, valueDistance);
        selectedValues[0] = firstValue;

        for (int i = 1; i < this.numberToSelect_; i++) {
            selectedValues[i] = firstValue + i * valueDistance;
        }

        int selectPointer = 0;
        int counter = 0;
        while (selectPointer < this.numberToSelect_) {
