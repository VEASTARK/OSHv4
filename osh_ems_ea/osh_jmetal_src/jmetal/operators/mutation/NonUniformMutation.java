...
public class NonUniformMutation extends Mutation {
    ...
    public void doMutation(double probability, Solution solution) throws JMException {
        ...
        for (int var = 0; var < solution.getDecisionVariables().length; var++) {
            if (this.pseudoRandom.randDouble() < probability) {
                double rand = this.pseudoRandom.randDouble();
                double tmp;

                if (rand <= 0.5) {
                    tmp = this.delta(x.getUpperBound(var) - x.getValue(var),
                            this.perturbation_);
                } else {
                    tmp = this.delta(x.getLowerBound(var) - x.getValue(var),
                            this.perturbation_);
                }
                tmp += x.getValue(var);
    ...
    private double delta(double y, double bMutationParameter) {
        double rand = this.pseudoRandom.randDouble();
        int it, maxIt;
        it = this.currentIteration_;
        maxIt = this.maxIterations_;
