...
public class SBXSinglePointCrossover extends Crossover {
    ...
    private static final List VALID_TYPES = Collections.singletonList(ArrayRealAndBinarySolutionType.class);
    ...
    public SBXSinglePointCrossover(HashMap<String, Object> parameters, PseudoRandom pseudoRandom) {
        ...
        if (parameters.get("realCrossoverProbability") != null)
            this.realCrossoverProbability_ = (Double) parameters.get("realCrossoverProbability");
        if (parameters.get("binaryrossoverProbability") != null)
            this.binaryCrossoverProbability_ = (Double) parameters.get("binaryrossoverProbability");
    ...
    public Solution[] doCrossover(Double realProbability,
        ...
        if (this.pseudoRandom.randDouble() <= realProbability) {
            for (int i = 0; i < x1.size(); i++) {
                valueX1 = x1.getValue(i);
                valueX2 = x2.getValue(i);
                if (this.pseudoRandom.randDouble() <= 0.5) {
                    if (java.lang.Math.abs(valueX1 - valueX2) > EPS) {
                        ...
                        rand = this.pseudoRandom.randDouble();
                        beta = 1.0 + (2.0 * (y1 - yL) / (y2 - y1));
                        alpha = 2.0 - java.lang.Math.pow(beta, -(this.distributionIndex_ + 1.0));

                        if (rand <= (1.0 / alpha)) {
                            betaq = java.lang.Math.pow((rand * alpha), (1.0 / (this.distributionIndex_ + 1.0)));
                        } else {
                            betaq = java.lang.Math.pow((1.0 / (2.0 - rand * alpha)), (1.0 / (this.distributionIndex_ + 1.0)));
                        } // if
        ...
        if (this.pseudoRandom.randDouble() <= binaryProbability) {
            ...
            int crossoverPoint = this.pseudoRandom.randInt(0, totalNumberOfBits - 1);
    ...
    public Object execute(Object object) throws JMException {
        ...
        Solution[] offSpring;
        offSpring = this.doCrossover(this.realCrossoverProbability_,
                this.binaryCrossoverProbability_, parents[0], parents[1]);
