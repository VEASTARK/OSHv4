...
public class DifferentialEvolutionCrossover extends Crossover {
    ...
    public Object execute(Object object) throws JMException {
        ...
        jrand = this.pseudoRandom.randInt(0, numberOfVariables - 1);

        // STEP 4. Checking the DE variant
        if ((this.DE_Variant_.compareTo("rand/1/bin") == 0) ||
                (this.DE_Variant_.compareTo("best/1/bin") == 0)) {
            for (int j = 0; j < numberOfVariables; j++) {
                if (this.pseudoRandom.randDouble(0, 1) < this.CR_ || j == jrand) {
                    double value;
                    value = xParent2.getValue(j) + this.F_ * (xParent0.getValue(j) -
                            xParent1.getValue(j));
          ...
          */
        ...
        else if ((this.DE_Variant_.compareTo("rand/1/exp") == 0) ||
                (this.DE_Variant_.compareTo("best/1/exp") == 0)) {
            for (int j = 0; j < numberOfVariables; j++) {
                if (this.pseudoRandom.randDouble(0, 1) < this.CR_ || j == jrand) {
                    double value;
                    value = xParent2.getValue(j) + this.F_ * (xParent0.getValue(j) -
        ...
        else if ((this.DE_Variant_.compareTo("current-to-rand/1") == 0) ||
                (this.DE_Variant_.compareTo("current-to-best/1") == 0)) {
            for (int j = 0; j < numberOfVariables; j++) {
                double value;
                value = xCurrent.getValue(j) + this.K_ * (xParent2.getValue(j) -
                        xCurrent.getValue(j)) +
                        this.F_ * (xParent0.getValue(j) - xParent1.getValue(j));
        ...
        else if ((this.DE_Variant_.compareTo("current-to-rand/1/bin") == 0) ||
                (this.DE_Variant_.compareTo("current-to-best/1/bin") == 0)) {
            for (int j = 0; j < numberOfVariables; j++) {
                if (this.pseudoRandom.randDouble(0, 1) < this.CR_ || j == jrand) {
                    double value;
                    value = xCurrent.getValue(j) + this.K_ * (xParent2.getValue(j) -
        ...
        else if ((this.DE_Variant_.compareTo("current-to-rand/1/exp") == 0) ||
                (this.DE_Variant_.compareTo("current-to-best/1/exp") == 0)) {
            for (int j = 0; j < numberOfVariables; j++) {
                if (this.pseudoRandom.randDouble(0, 1) < this.CR_ || j == jrand) {
                    double value;
                    value = xCurrent.getValue(j) + this.K_ * (xParent2.getValue(j) -
            ...
            Configuration.logger_.severe("DifferentialEvolutionCrossover.execute: " +
                    " unknown DE variant (" + this.DE_Variant_ + ")");
