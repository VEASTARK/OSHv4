...
public class Spea2Fitness {
     ...
     */
    private final SolutionSet solutionSet_;
    ...
    public void fitnessAssign() {
        ...
        for (int i = 0; i < this.solutionSet_.size(); i++) {
            for (int j = 0; j < this.solutionSet_.size(); j++) {
                if (dominance_.compare(this.solutionSet_.get(i), this.solutionSet_.get(j)) < 0) {
                    strength[i] += 1.0;
                } // if
            } // for
        } // for
        ...
        for (int i = 0; i < this.distance.length; i++) {
            Arrays.sort(this.distance[i]);
            kDistance = 1.0 / (this.distance[i][k] + 2.0); // Calcule de D(i) distance
            //population.get(i).setFitness(rawFitness[i]);
            this.solutionSet_.get(i).setFitness(rawFitness[i] + kDistance);
        } // for
    } // fitnessAsign
    ...
    public SolutionSet environmentalSelection(int size) {
        ...
        while (aux.size() > size) {
            ...
            for (List<DistanceNode> aDistanceList : distanceList) {
                // if
                // while
                aDistanceList.removeIf(distanceNode -> distanceNode.getReference() == tmp);
            } // while
        } // while
        return aux;
