...
public class R2 {
    ...
    public R2(int nObj, String file) {
        ...
        try {
            ...
            int index = 0;
            for (double[] aList : list) this.lambda_[index++] = aList;
    ...
    public static void main(String[] args) {
        ...
        //Obtain delta value
        double value = qualityIndicator.R2Front(approximationFront, paretoFront);
    ...
    R2Withouth(double[][] approximation, double[][] paretoFront, int index) {
        ...
        maximumValue = this.utils_.getMaximumValues(paretoFront, this.nObj_);
        minimumValue = this.utils_.getMinimumValues(paretoFront, this.nObj_);

        // STEP 2. Get the normalized front and true Pareto fronts
        normalizedApproximation = this.utils_.getNormalizedFront(approximation,
                maximumValue,
                minimumValue);
        normalizedParetoFront = this.utils_.getNormalizedFront(paretoFront,
                maximumValue,
                minimumValue);
        ...
        for (int i = 0; i < this.lambda_.length; i++) {
            ...
            double tmp;
            if (index != 0) {
                tmp = this.matrix_[0][i];
            } else {
                tmp = this.matrix_[1][i];
            }
        ...
        return sumWithout / this.lambda_.length;
     ...
     */
    public double
    R2Front(double[][] approximation, double[][] paretoFront) {
        ...
        // STEP 5. Return the R2 value
        return sum / this.lambda_.length;
     ...
     */
    public double
    R2Pop(SolutionSet set) {
        double[][] approximationFront = set.writeObjectivesToMatrix();
        double[][] trueFront = set.writeObjectivesToMatrix();

        return this.R2Front(approximationFront, trueFront);
