...
public class InvertedGenerationalDistance {
    ...
    public final jmetal.qualityIndicator.util.MetricsUtil utils_;  //utils_ is used to access to the
    //distances
    ...
    public double invertedGenerationalDistance(double[][] front,
        ...
        maximumValue = this.utils_.getMaximumValues(trueParetoFront, numberOfObjectives);
        minimumValue = this.utils_.getMinimumValues(trueParetoFront, numberOfObjectives);

        // STEP 2. Get the normalized front and true Pareto fronts
        normalizedFront = this.utils_.getNormalizedFront(front,
                maximumValue,
                minimumValue);
        normalizedParetoFront = this.utils_.getNormalizedFront(trueParetoFront,
                maximumValue,
                minimumValue);
        ...
        for (double[] aNormalizedParetoFront : normalizedParetoFront)
            sum += Math.pow(this.utils_.distanceToClosedPoint(aNormalizedParetoFront,
                    normalizedFront),
                    pow_);
        ...
        return sum / normalizedParetoFront.length;
    } // generationalDistance
