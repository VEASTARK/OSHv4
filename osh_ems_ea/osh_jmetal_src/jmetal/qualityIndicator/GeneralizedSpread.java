...
public class GeneralizedSpread {
    ...
    public double generalizedSpread(double[][] paretoFront,
        ...
        for (int i = 0; i < numberOfObjectives; i++) {
            ...
            System.arraycopy(normalizedParetoFront[normalizedParetoFront.length - 1], 0, extremeValues[i], 0, numberOfObjectives);
        }
        ...
        if (utils_.distance(normalizedFront[0], normalizedFront[normalizedFront.length - 1]) == 0.0) {
            ...
            dMean /= (numberOfPoints);

            // STEP 7. Calculate the distance to extremal values
            double dExtremes = 0.0;
            for (double[] extremeValue : extremeValues) {
                dExtremes += utils_.distanceToClosedPoint(extremeValue, normalizedFront);
            }
            ...
            return (dExtremes + mean) / (dExtremes + (numberOfPoints * dMean));
