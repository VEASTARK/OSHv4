	...
	public void initializeInterdependentCalculation(long maxReferenceTime,
		...
		if (outputStatesCalculatedFor != maxReferenceTime) {
			...
			while (time < maxHorizon) {				
				...
				double actPower = baseload.getAverageLoadFromTill(Commodity.ACTIVEPOWER, time, time + stepSize);
				double reactPower = baseload.getAverageLoadFromTill(Commodity.REACTIVEPOWER, time, time + stepSize);
